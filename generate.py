...
def write_digest_js():
    """Write digest.js with config vars and all page JS.
    Keeping JS in a separate file prevents email HTML content from
    corrupting the script parser (unterminated regex / syntax errors).
    """
    js = """// Daily Digest — page interactions
// Auto-generated by GitHub Actions. Do not edit manually.
var GH_OWNER = {gh_owner};
var GH_REPO  = {gh_repo};
var GH_PAT   = {gh_pat};
...
""".format(
        gh_owner=json.dumps(GH_OWNER),
        gh_repo=json.dumps(GH_REPO),
        gh_pat=json.dumps(GH_PAT),
    )
    with open("digest.js", "w") as f:
        f.write(js)
    print("  digest.js written")
